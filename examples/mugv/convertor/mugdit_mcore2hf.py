...
def convert_mcore2hf(
    ...
    for name in keys:
        ...
        if "start_token" in name:
            ...
            new_name = name
        elif "timestep_embedder" in name:
            new_name = name.replace("timestep_embedder", "t_embedder")
        elif "fps_embedder" in name:
            new_name = name
        elif "timestep_block" in name:
            new_name = name.replace("timestep_block", "t_block")
        ...
        if "decoder.layers" in name:
            for layer_idx in range(num_layers):
                ...
                elif ".pre_cross_attn_layernorm.weight" in name:
                    new_name = f"{prefix}.cross_attn_norm.weight"
                elif ".pre_cross_attn_layernorm.bias" in name:
                    new_name = f"{prefix}.cross_attn_norm.bias"
                ...
                elif ".cross_attention.q_layernorm.weight" in name:
                    new_name = f"{prefix}.cross_attn.q_norm.weight"
                elif (
                    ".cross_attention.q_layernorm.bias" in name
                ):  # default is RMSnorm, no bias
                    new_name = f"{prefix}.cross_attn.q_norm.bias"
                elif ".cross_attention.k_layernorm.weight" in name:
                    new_name = f"{prefix}.cross_attn.k_norm.weight"
                elif ".cross_attention.k_layernorm.bias" in name:
                    new_name = f"{prefix}.cross_attn.k_norm.bias"
...
Example usage:
python -m examples.mugv.convertor.mugdit_mcore2hf --dcp-dir /dcp/ckpt/folder --output /torch/save/file --model-size 10B
""",
