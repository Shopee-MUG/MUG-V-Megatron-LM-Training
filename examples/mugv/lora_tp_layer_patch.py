...
class LoraParallelLinear(MegatronModule, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(config=kwargs.get("megatron_config", None))
        LoraLayer.__init__(self, base_layer=base_layer, **kwargs)
        self.lora_bias = defaultdict(lambda: None)
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    if isinstance(lora_A, nn.Linear):
                        result = result + lora_B(lora_A(dropout(x)))[0] * scaling
                    elif isinstance(lora_B, nn.Linear):
                        result = result + lora_B(lora_A(dropout(x))[0]) * scaling
                    else:
                        raise ValueError(f"Lora result is not a tuple: {lora_res}")
                else:
