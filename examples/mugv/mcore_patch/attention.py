...
#
# This file is modified from Megatron-Core's attention.py to support:
# - QK LayerNorm for improved training stability
# - Custom cross-attention for text conditioning in diffusion models
...
@dataclass
class CrossAttentionQKNormSubmodules(CrossAttentionSubmodules):
...
class SelfAttention(Attention):
    ...
    def forward(
        ...
        *,
        ...
        if self.config.enable_start_end_tokens and self.config.sequence_parallel:
            query, key, value = self._trim_start_end_tokens(query, key, value)
        ...
        if self.config.enable_start_end_tokens and self.config.sequence_parallel:
            core_attn_out = self._pad_start_end_tokens(core_attn_out)
    ...
    def _trim_start_end_tokens(self, query, key, value):
        # if self.config.enable_start_end_token:

        def _trim_start_end_tokens_for_tensor(tensor, seq_dim=0):
            start, end = tensor[:1], tensor[-1:]
            per_seq_length = tensor.size(seq_dim) // get_tensor_model_parallel_world_size()
            seq_list = tensor.split(per_seq_length, dim=seq_dim)
            clean_seq = [seq[1:-1] for seq in seq_list]
            result = torch.cat([start, *clean_seq, end], dim=seq_dim)
            return result

        query = _trim_start_end_tokens_for_tensor(query).contiguous()
        key = _trim_start_end_tokens_for_tensor(key).contiguous()
        value = _trim_start_end_tokens_for_tensor(value).contiguous()
        return query, key, value

    def _pad_start_end_tokens(self, hidden_states):

        def _pad_start_end_tokens_for_tensor(tensor, seq_dim=0):
            start, end = tensor[:1], tensor[-1:]
            per_seq_length = tensor.size(seq_dim) // get_tensor_model_parallel_world_size()
            seq_list = tensor[1:-1].split(per_seq_length, dim=seq_dim)
            seq_list = [torch.cat([start, seq, end], dim=seq_dim) for seq in seq_list]
            result = torch.cat(seq_list, dim=seq_dim)
            return result

        hidden_states = _pad_start_end_tokens_for_tensor(hidden_states).contiguous()
        return hidden_states


class CrossAttentionQKNorm(Attention):
