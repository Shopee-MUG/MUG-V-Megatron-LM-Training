...
#
# This file is modified from megatron/core/transformer/transformer_block.py to support:
# - diffusion transformers blocks
...
def _get_block_submodules(
    ...
    spec: Union[MUGDiTBlockSubmodules, ModuleSpec],
    vp_stage: Optional[int] = None,
) -> MUGDiTBlockSubmodules:

    # Transformer block submodules.
    if isinstance(spec, MUGDiTBlockSubmodules):
        return spec
    ...
    # `BaseTransformerLayer` from the `transformer_layer.py` file.
        ...
        if issubclass(spec.module, MUGDiTBlock):
            return spec.submodules
        elif issubclass(spec.module, BaseTransformerLayer):
            num_layers = get_num_layers_to_build(config, vp_stage)
            return MUGDiTBlockSubmodules(
...
class MUGDiTBlock(MegatronModule):
    """MUGDiT class, adapted from Transformer class.

    # Note: currently not supports cuda graph due to
        the variable shape of the input tensors
    """
    ...
    def _build_layers(self):
        ...
        # TODO: In MUGDiT, final_layer consists of a LN and a MLP and is handled in the outter forward function, so we disbale it here.
    ...
    def _checkpointed_forward(
        ...
        def custom(start: int, end: int):
            def custom_forward(
                hidden_states, attention_mask, context, context_mask, mask_timesteps, rotary_pos_emb
            ):
    ...
    def forward(
        ...
        *,
        ...
        """
        Perform the forward pass through the MUGDiT block.
    ...
    def sharded_state_dict(
        ...
        """
        Generate a sharded state dictionary for the MUGDiT block.
